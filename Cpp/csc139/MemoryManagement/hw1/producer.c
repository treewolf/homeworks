...
Fong, Joshua
Section #01
OSs Tested on: Arch, Redhat(athena)
*/
...
void InitShm(int bufSize, int itemCnt)
{
	int in = 0;
    int out = 0;
	const char *name = "OS_HW1_JoshF"; // Name of shared memory object to be passed to shm_open
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
    ...
    srand(randSeed);
   	...
   	// Write code here to produce itemCnt integer values in the range [0-5000]
