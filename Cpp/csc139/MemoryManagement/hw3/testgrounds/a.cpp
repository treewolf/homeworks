...
void PriorityWithPre(){
	...
	while(completed.size() != alg.number_of_proc){
		...
		while(!ready.empty()){
			...
			//if original struct is same front, then no need to print
		//	if(ready.front().flag_repeated == 0)
			if(ready.front().flag_repeated.back() != timer-1)
				out << timer << "\t" << ready.front().proc_num << std::endl;
			ready.front().flag_repeated.push_back(timer);
			
			bool task_unfinished;
			struct Qu tt;
			...
			//add sorted into ready
			priority_sort("priority");		
	
			//find original task if unfinished, and if priority equals the one
			//after, then add new first, then add original
//			std::queue<Qu> tempq;
