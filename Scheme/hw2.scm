...
(begin
	...
	(define (lastTwo x)
		(if (>= x 100) (lastTwo (modulo x 100)) x))

	; x - integer
	; return last digit of integer
	(define (lastDigit x)
		(if (>= x 10) (lastDigit (modulo x 10)) x))
	...
	(define (inc-by-n x d) (inc x d (len x 0) 0 0))
	...
	(define (len x n) (if (< x 1) n (len (quotient x 10) (+ n 1))))

	; x - integer
	; p - the given power of
	; will add p-1 0's to end
	; returns power of 10 : x * 10^p
	(define (powerTen x p) (if (= p 0) x (powerTen (* x 10) (- p 1))))	
	...
	(define (mapeven f L)
		(cond
			((null? L) '())
			((list? L)
				(cons (car L) (cons (f (car (cdr L))) (mapeven f (cdr (cdr L))))))))
	...
	(define (addTheSquares L)
		(if (null? L) 0 (+ (* (car L) (car L)) (addTheSquares (cdr L)))))

	; 6.
	; L - list of elements
	; x - element to append
	; return list of a function with integer after each element
	(define (makeInsertInList x) (lambda (L) (inThis L x))) 

	; aux for makeinsertinlist
	; L - list
	; x - element
	; return list of function with integer after each element	
	(define (inThis L x) (if (null? L) '() (append (cons (car L) (list x)) (inThis (cdr L) x))))
